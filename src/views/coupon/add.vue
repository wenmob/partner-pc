  ...
  <div class="app-container">
    ...
    >
      ...
      >
        ...
        >
          <el-col :span="24">
            <el-radio :label="-1">全国适用</el-radio>
          </el-col>
          <el-col
            :span="24"
...
<script>
import { couponsAdd, couponsShowCarModelList, couponsInitUpdateCoupons, couponsUpdate } from "@/api/coupon";

export default {
  data() {
    const validateMinusAmount = (rule, value, callback) => {
         if (value !== "") {
             if ((/^[+]{0,1}(\d+)$|^[+]{0,1}(\d+\.\d+)$/).test(value) === false || value * 1 === 0) {
    ...
    return {
      ...
      ruleTest: {
        ...
        fullAmount: [
          { required: true, message: "请输入满多少金额后使用", trigger: "blur" },
  ...
  methods: {
    ...
    chooseTime() {
      if (this.validTime && this.validTime.length > 0) {
        this.formTest.validStartTime = this.validTime[0];
        this.formTest.validEndTime = this.validTime[1];
      } else {
        this.formTest.validStartTime = "";
        this.formTest.validEndTime = "";
      }
